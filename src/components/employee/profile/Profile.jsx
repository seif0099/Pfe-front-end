...
function Profile() {
  ...
  async function uploadImg() {
    ...
    axios
      .post("http://localhost:9000/updateImage?id=" + userInfo?._id, formData, config)
      .then((response) => {
      ...
      <div className="wrapper">
        <div className="inner inner1">
          <form>
            ...
            <div className="divider">
              {touched.nom && errors.nom ? <p className="errors lDivider">{errors.nom}</p> : null}
              {touched.prenom && errors.prenom ? <p className="errors rDivider">{errors.prenom}</p> : null}
            </div>

            <div className="field1">
              <input type="text" name="tel" onChange={handleChange} placeholder={userInfo.tel} />
            </div>
            {touched.tel && errors.tel ? <p className="errors">{errors.tel}</p> : null}
            <div className="field1">
              <input type="text" name="adresse" onChange={handleChange} placeholder={userInfo.adresse} />
            </div>
            {touched.adresse && errors.adresse ? <p className="errors">{errors.adresse}</p> : null}
            <div className="field1">
              <input type="text" name="sitFam" onChange={handleChange} placeholder={userInfo.sitFam} />
            </div>
            {touched.sitFam && errors.sitFam ? <p className="errors">{errors.sitFam}</p> : null}
